...
# Force datasets to use soundfile for audio decoding rather than torchcodec,
# which requires PyTorch. Must be set before 'datasets' is imported.
...
# ── Paths (shared with the alignment pipeline) ──────────────────────────────────
...
def run_align(overwrite: bool) -> bool:
    cmd = [sys.executable, "-m", "twi_aligner.align"]
    if overwrite:
